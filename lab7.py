...
class Term:
pass
# In App, function names are always considered to be
constants, not variables.
# This simplifies things and doesn't affect expressivity. We
can always model
# variable functions by envisioning an apply(FUNCNAME, ...
args ...).
...
self.args = args
def __str__(self):
return '{0}({1})'.format(self.fname, ','.join(map(str,
self.args)))
def __eq__(self, other):
return (type(self) == type(other) and
self.fname == other.fname and
all(self.args[i] == other.args[i] for i in
range(len(self.args))))
__repr__ = __str__
class Var(Term):
...
return self.value
def __eq__(self, other):
return type(self) == type(other) and self.value ==
other.value
...
class TermParser:
"""Term parser.
Use the top-level parse_term() instead of instantiating this
class directly.
"""
...
# We have to look at the next token to distinguish between
App and
# Var.
...
Returns a subst (map of name->term) that unifies x and y, or
None if
they can't be unified. Pass subst={} if no subst are
initially
...
Returns a term where all occurrences of variables bound in
subst
