...
def definiteNoun(s):
s = s.lower().strip()
if s in ['a', 'e', 'i', 'o', 'u', 'y']: return "an " + s
else:
return "a " + s
def removeArticle(s):
...
if yesAnswer:
q = makeQuestion(newQuestion, correctAnimal,
wrongAnimal)

else:
q = makeQuestion(newQuestion, wrongAnimal,
correctAnimal)
Q = replaceAnswer(Q, wrongAnimal, q)
tries = 0
return Q

def addNewQuestion(wrongAnimal, newques, correct): global Q
q = makeQuestion(newques, correct, wrongAnimal) Q =
replaceAnswer(Q, wrongAnimal, q)
return Q
...
print ("You are only allowed to answer YES or NO.") print ("")
